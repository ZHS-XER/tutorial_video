...
export const Main: React.FC<TutorialProps> = ({ bgm = false, vo = true, captions = true }) => {
  ...
  return (
    <AbsoluteFill style={{ background: C.ink, fontFamily: F.sans }}>
      <Scene from={S.title.from} dur={S.title.dur}>{(su) => <S0Title u={su} title="Workflow basics" ep="EP 01" steps={CHAPTERS.map((c) => ({ n: c.n, title: c.title }))} previewSrc={VIDEO} />}</Scene>
      {CHAPTERS.map((c) => (
