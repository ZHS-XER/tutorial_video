...
- "选中多个节点一起连线"的共用输出点：多选态是真实快照，圆点按 2026-09-17 在产品上实采的样式（深底圆点、按输出类型分段的蓝绿彩环、数字 2）注入画面，拖出的蓝色连线是合成叠加；产品实测拖它一次接上两条线。
...
- 同一支片子另出了中文界面 + 中文字幕 + 中文配音版 `out/ep1-basics-zh.mp4`（无配音版 `ep1-basics-zh-novo.mp4`）。镜头、节奏、总时长与英文版完全一致；片头大标题、七张章节卡、结尾快捷键回顾、接口颜色图例、键帽提示都换成中文。
- 产品界面用的是 YouArt 官方中文文案（从产品中文版原样取的翻译）和官方中文字体 MiSans，不是重新录屏：菜单、按钮、节点名、设置面板在渲染时逐条替换。字幕和配音里的按钮、节点名称跟界面一致（运行全部、适应视图、媒体资产、收起全部节点……），快捷键念 "Command 加左方括号" 这类说法。模型名、示例提示词、文件名保持原样。
- 中文配音仍是 Chris 声线（ElevenLabs v3 中文）。为了让中文句子落在英文原拍位上，台词写得比英文精简，个别列举句压缩了句内停顿或轻微加速。


## v3.6 更新（2026-09-17 晚）

- 第 2 节"选中两个源节点一起连线"的共用输出点换成产品真实样式：深底圆点、按输出类型分段的蓝绿彩环、数字 2，位于选区右缘垂直居中；拖它时两条源输出的蓝线汇入圆点、再引一条蓝线到 Seedance，落下两条线同时接上。样式来自在产品上实际操作采到的快照（`captures/ep1-basics-s2-multisel-new.html`）。
- 结尾成片视频改从第 3 秒起播，说 "That's all for EP1" 时招牌上的 YouArt 已基本成形。
- 英文、中文及两个无配音版四个成片同步更新，总长仍为 5 分 53 秒。结尾 storyboard 帧 `out/storyboard/s8.png` 已刷新。
- 留档说明：本片全部产品画面采自网站给自动化浏览器（UA 含 HeadlessChrome）的旧版界面，与真实用户当前看到的新版节点样式有差异。操作逻辑与快捷键一致；是否按新版界面重采整支片子另行排期。
- 飞书文档同步追加了同名一节，并改写了"与大纲的出入"里关于共用输出点的那条。
