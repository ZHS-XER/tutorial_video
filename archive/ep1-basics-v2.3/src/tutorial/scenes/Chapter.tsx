// Chapter — 章节卡：大号编号 + 标题 + 副标题，黄条从左扫入；底部 7 段进度轨道点亮当前章（与片头轨道呼应）。45 帧。
import React from 'react';
import { C, E, F, T } from '../../../film/tokens';
// 2026-09-10：去掉 "0n / 07" 小字（无信息量），进度条本身放大。
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: C.ink, opacity: out }}>
      <div style={{ position: 'absolute', left: 240, top: 0, bottom: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 18 }}>
        ...
        <div style={{ height: 5, width: 560 * bar, background: C.accent, borderRadius: 3, marginLeft: 0 }} />
        <div style={{ fontSize: 42, lineHeight: 1.2, fontWeight: 500, letterSpacing: '-0.01em', color: 'rgba(250,250,250,0.72)', fontFamily: F.sans, opacity: seg(u, 12, 26, E.zoom), transform: `translateY(${(1 - seg(u, 12, 26, E.zoom)) * 10}px)` }}>{sub}</div>
      </div>
      {/* 进度轨道：已讲过的章亮灰，当前章黄，未讲的暗 */}
